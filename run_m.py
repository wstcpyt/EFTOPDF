...
iqewl = np.array([])
for wlobject in wl:
    file = open('pdf_m/%d_NORMAL_PDF.dat'%(wlobject))
    gf = np.array([])
    gffoward = np.array([])
    for line in file:
...
AM = np.zeros(s)
for i in range(0,n):
    search = open('pdf_m/%d_NORMAL_PDF.dat'%(wl[i]))
    rawAMarray = np.array([])
    AMarray = np.array([])
    AMfoward = np.array([])
    for line in search:
...
plt.plot(xx/17.0,smoothed,linewidth=3.0,label='Gaussian smooth')
...
legend = ax1.legend(loc='upper right', shadow=True,prop={'size':20})
show()